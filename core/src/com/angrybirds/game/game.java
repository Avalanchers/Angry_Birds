...
public class game implements Screen {
	...
	public void render(float delta) {
		...
		if (Gdx.input.getPressure() == 1 && pausePhysics && includes((yellowBird) player,
				new Vector2(Gdx.input.getX() / 2, (Gdx.graphics.getHeight() - Gdx.input.getY()) / 2))) {
			hold = true;
			if (mouseOrigin == null)
		...
		for (int i = 0; i < NumOfObjects; i++) {
			Wood1Sprite.setBounds(levelObjects[i].body.getPosition().x - Wood1Sprite.getWidth() / (2),
					levelObjects[i].body.getPosition().y - Wood1Sprite.getHeight() / (2),
					2 * (levelObjects[i].width / PPM), 2 * (levelObjects[i].height / PPM));
		...
		for (int i = 0; i < 5; i++) {
			test.setBounds(enemies[i].body.getPosition().x - test.getWidth() / (2),
					enemies[i].body.getPosition().y - test.getHeight() / (2), 2 * (enemies[i].width / PPM),
					2 * (enemies[i].height / PPM));
			test.setOrigin(test.getWidth() / 2, test.getHeight() / 2);
			test.setRotation(MathUtils.radiansToDegrees * enemies[i].body.getAngle());
			test.draw(batch);
		}
		player.sprite.setBounds(player.body.getPosition().x - player.sprite.getWidth() / (2),
				player.body.getPosition().y - player.sprite.getHeight() / (2), 2 * (player.width / PPM),
				2 * (player.height / PPM));
		...
		if (Gdx.input.getY() >= 28 && Gdx.input.getY() <= 74 && Gdx.input.getX() >= 32 && Gdx.input.getX() <= 125
				&& Gdx.input.isTouched()) {
	...
	public void getMouseInput() {
		System.out.println(Gdx.input.getX() / PPM + " " + (Gdx.graphics.getHeight() - Gdx.input.getY()) / PPM + " "
				+ player.body.getPosition().x + " " + player.body.getPosition().y);

		if (Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
			System.out.println("EFregrh");
			player.body.setTransform((float) Gdx.input.getX() / PPM / 2,
					(float) (Gdx.graphics.getHeight() - Gdx.input.getY()) / PPM / 2, 0);
		}
		if (Gdx.input.getPressure() == 0) {
			hold = false;
			pausePhysics = false;
			player.body.setLinearVelocity(((mouseOrigin.x - Gdx.input.getX()) / 8),
					(mouseOrigin.y - (Gdx.graphics.getHeight() - Gdx.input.getY())) / 4);
		}
	}

	public boolean includes(yellowBird player, Vector2 mousePoint) {
		if (mousePoint.x > player.body.getPosition().x * PPM - (player.width)
				&& mousePoint.x < player.body.getPosition().x * PPM + (player.width)
				&& mousePoint.y > player.body.getPosition().y * PPM - (player.height)
				&& mousePoint.y < player.body.getPosition().y * PPM + (player.height))
			return true;
		else
			return false;
